...
version:    24.12.29.12.30 (Modified with fixes)
'''
...
from selenium.common.exceptions import (
    ...
    WebDriverException,
    StaleElementReferenceException  # ADDED FOR FIX
)
...
#< Helper functions for stale element handling (NEW)
def safe_get_text(element: WebElement, max_retries: int = 3) -> str:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                print_lg(f"Stale element in safe_get_text, retrying... (Attempt {attempt + 1}/{max_retries})")
                buffer(0.5)
                continue
            raise
    return ""
...
def safe_click(element: WebElement, max_retries: int = 3) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                print_lg(f"Stale element in safe_click, retrying... (Attempt {attempt + 1}/{max_retries})")
                buffer(0.5)
                continue
            raise
    return False
...
def safe_get_attribute(element: WebElement, attribute: str, max_retries: int = 3) -> str:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                print_lg(f"Stale element in safe_get_attribute, retrying... (Attempt {attempt + 1}/{max_retries})")
                buffer(0.5)
                continue
            raise
    return ""
...
def get_job_main_details(job: WebElement, blacklisted_companies: set, rejected_jobs: set) -> tuple[str, str, str, str, str, bool]:
    '''
    # Function to get job main details with stale element handling.
    Returns a tuple of (job_id, title, company, work_location, work_style, skip)
    * job_id: Job ID
    * title: Job title
    * company: Company name
    ...
    * skip: A boolean flag to skip this job
    ...
    for attempt in range(max_retries):
        try:
            ...
            scroll_to_view(driver, job_details_button, True)
            job_id = safe_get_attribute(job, 'data-occludable-job-id')
            
            title = safe_get_text(job_details_button)
            title = title[:title.find("\n")] if "\n" in title else title
            
            other_details_element = job.find_element(By.CLASS_NAME, 'artdeco-entity-lockup__subtitle')
            other_details = safe_get_text(other_details_element)
            
            index = other_details.find(' · ')
            company = other_details[:index] if index != -1 else other_details
            work_location = other_details[index+3:] if index != -1 else ""
            
            # Extract work style safely
            ...
            try:
                ...
                if safe_get_text(footer_state) == "Applied":
                    skip = True
                    print_lg(f'Already applied to "{title} | {company}" job. Job ID: {job_id}!')
            except: 
                pass
            ...
            try: 
                if not skip: 
                    safe_click(job_details_button)
            except Exception as e:
                print_lg(f'Failed to click "{title} | {company}" job on details button. Job ID: {job_id}!') 
                discard_job()
                safe_click(job_details_button)  # To pass the error outside
            ...
            if attempt < max_retries - 1:
                print_lg(f"Stale element encountered in get_job_main_details, retrying... (Attempt {attempt + 1}/{max_retries})")
                buffer(2)  # Wait before retry
                continue
            else:
                print_lg(f"Failed to get job details after {max_retries} attempts")
                raise
        except Exception as e:
            print_lg(f"Unexpected error in get_job_main_details: {e}")
            raise
...
def apply_to_jobs(search_terms: list[str]) -> None:
    ...
    for searchTerm in search_terms:
        ...
        try:
            ...
            print_lg(f"Failed to apply filters: {e}")
            print_lg("Continuing without filters...")

        current_count = 0
        try:
            while current_count < switch_number:
                # Wait until job listings are loaded with retry logic
                max_wait_retries = 3
                for wait_attempt in range(max_wait_retries):
                    try:
                        ...
                        if wait_attempt < max_wait_retries - 1:
                            print_lg(f"Waiting for job listings, retry {wait_attempt + 1}/{max_wait_retries}")
                            buffer(3)
                        else:
                            raise
                ...
                for job_index, job in enumerate(job_listings):
                    ...
                    try:
                        ...
                        print_lg(f"Skipping job at index {job_index} due to stale element")
                        continue
                    except Exception as e:
                        print_lg(f"Error getting job details at index {job_index}: {e}")
                        continue
                ...
                try:
                    ...
                    # Scroll to the button and click with retry
                    for click_attempt in range(3):
                        try:
                            ...
                            print_lg(f"\n>-> Now on Page {current_page+1} \n")
                            buffer(3)  # Wait for page to load
                            break
                        except StaleElementReferenceException:
                            if click_attempt < 2:
                                print_lg("Stale element on page navigation, retrying...")
                                # Re-find the pagination element
                                pagination_element, current_page = get_page_info()
                                next_page_button = pagination_element.find_element(
                                    By.XPATH, f"//button[@aria-label='Page {current_page+1}']"
                                )
